...
| **同步後端 (Sync Backends)** | 可插拔 Provider 註冊機制 — Books、Collections、Settings 全部走同一個 `SyncBackend` 介面。內建兩個 Backend:**Google Drive**(App Data folder)與 **S3-compatible**(`@aws-sdk/client-s3`,lazy-loaded,使用 Drive 時 SDK 不進 initial bundle)。雙向同步以 `lastActiveAt` / `updatedAt` 做 last-write-wins,搭配 pending-deletion 追蹤確保刪除能傳遞。 |
