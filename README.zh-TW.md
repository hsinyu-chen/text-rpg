...
TextRPG 是一個**本地優先 (Local-First)**、**自帶金鑰 (Bring Your Own Key)** 的 Web 應用程式，圍繞長窗口 LLM 而設計（也支援打包為 Tauri 桌面版，見 [部署指南](#部署指南)）。它不同於傳統的 AI 聊天機器人，而是將 LLM 視為一個嚴謹的「地下城主 (DM)」，透過結構化思考與邏輯判定推進劇情，並把遊戲狀態（物品、任務、劇情摘要）持久化於本地 Markdown 檔案中。透過 JSON Schema 強制模型執行「判定優先」的流程。
...
*   **Android Chrome / 桌面 Chrome / Edge**：瀏覽器選單（三個點）→ **Install app** / **Add to Home screen**。
*   **iOS Safari**：分享按鈕 → **Add to Home Screen**。iOS 沒有 Battery Status API，App 內的電量指示會隱藏，其他功能正常。
...
*   **Smart Context**: 動態組裝「劇情大綱（Markdown）」+「完整對話歷史」。
...
> - **NPC 主動性鐵則**——透過「動作 / 表情·眼神 / 台詞或內心獨白」三層推演避免 NPC 變成沉默的背景板。每位在場 NPC 與每個關鍵物件的覆蓋本身由結構化分析 schema(`npc_reactions[]` / `object_reactions[]`)上游強制保證。
...
存檔不只記錄進度，還會**更新世界設定檔**：
*   **觸發方式**:
    1. 點擊輸入框旁的 **Save**（磁碟片圖示）按鈕 → 確認 dialog → 存檔直接執行。
    2. 也可在訊息工具列點 **"Auto Update"**（魔術棒圖示）— 針對既有訊息重新跑同樣的 manifest → dispatcher 流程。
...
*   **進入方式**: 點擊側邊欄的 **"檢視檔案"**(資料夾圖示)按鈕。
*   **功能**: 開啟 **File Viewer** 視窗,左側列出所有載入的 Markdown 檔案,中央是 Monaco 編輯器。
*   **編輯**: 檔案隨時可編輯 — 在左側點選後直接打字即可。有未儲存編輯的檔案會在清單顯示標記,切換檔案時不會遺漏進行中的修改。
*   **儲存**: 點擊對話框右上角的 **Save Changes** 把編輯寫進 IndexedDB,引擎下一回合就讀得到;若有未儲存編輯時關閉對話框會跳確認。
*   **大綱導航**: 對話框底部提供當前檔案的 Markdown **Outline**(左下切換),點章節即可跳轉。
*   **跨檔搜尋取代**: 左側另有 **Search** 分頁,支援 regex / 大小寫 / 全字匹配等選項。

### 5. 內建 AI Agent(KB 編輯 + Q&A + UI 引導)

嵌在遊戲內的 LLM 驅動 Agent。可以幫你改 KB 檔、回答關於世界 / 機制 / 對話歷史的問題、引導你到還沒找到的 UI 功能。它用自己的 LLM Profile(與主遊戲分離),所以你可以指派一個小而便宜的模型負責編輯,主遊戲跑更重的模型敘事。

Agent 有 **兩個 surface**,共用同一套服務:
...
*   執行紀錄會顯示使用者輸入、模型回覆(Markdown 渲染)、思考過程、Tool 呼叫、Tool 結果 — 每一段可獨立摺疊。
...
3.  **建立 OAuth 2.0 用戶端 ID** — 視你跑的版本：
    ...
    *   **Tauri 桌面版**：建立 **Desktop app** 類型 client ID。**Tauri PKCE flow 不能用網頁應用程式類型** — 必須用 Desktop 類型搭配 client secret 才能 token exchange。
4.  **提供憑證**：
    *   **Web** — 擇一即可：
...
TextRPG 將語言拆成**兩個獨立設定**：

#### 1. 介面語言（UI Chrome）
*   **位置**：Settings → Game Settings → Interface Language
*   **可選值**：`Follow system`（預設）／`繁體中文`／`English`
*   **真實來源**：`AppConfigStore.interfaceLanguage`，KV key 為 `app_interface_language`
*   **`'system'` 解析**：在讀取時走 `UI_LOCALES` 註冊表的 `matchPrefixes` 比對 `navigator.language`，所以 OS locale 改變不需要重寫設定值就會跟著生效
*   **影響範圍**：對話框標題、按鈕、意圖標籤、placeholder、snackbar、Settings 對話框本身 —— 所有透過 `I18nService` / `TranslatePipe` 即時渲染的文字

#### 2. 故事（輸出）語言（LLM 綁定）
*   **位置**：Settings → Game Settings → Story Language
*   **可選值**：自由字串 —— 可從預設清單選（`Traditional Chinese`、`English`）、`default`，或自填任意字串（如 `Klingon`）；LLM 會被告知用該字串對應的語言寫故事
*   **影響範圍**：
    *   AI 生成的故事內容語言
    *   `summary`、`inventory_log`、`quest_log`、`world_log` 等結構化輸出
    *   系統文件名稱（如 `2.Story_Outline.md` vs `2.劇情綱要.md`）
    *   包在使用者輸入外面的 `<intent>` XML marker（`<Action>` vs `<行動意圖>`）
    *   分析軌跡的 markdown 標籤（會跟訊息一起存進 chat history）
...
#### 3. 混合語言組合
這兩個設定是**故意解耦**的，所以中文母語的使用者想練英文時可以設定 `Interface Language = 繁體中文` + `Story Language = English`（或反過來）而不會打架。

> [!WARNING]
> 不過劇情中途切換 **Story Language** 仍有風險：歷史訊息保留原語言、角色名/地名可能在切換點前後不一致，劇本檔本身也只有一種語言版本。建議在開新遊戲前就先決定好故事語言；介面語言可以隨時切。

#### 4. 切換注意事項
*   **現有遊戲**：切換 Story Language 後，新 AI 回應會使用新語言，但歷史訊息保持原語言
*   **劇本相容性**：確保選擇的劇本有對應 Story Language 版本（參見下方本地化指南）
*   **自訂 Story Language**：填一個 UI 字典沒有的字串（例如 `Klingon`）只會影響 LLM 寫作；UI 仍依 Interface Language 渲染

---

## 本地化指南

TextRPG 用**兩層**系統，各自由上述兩個設定驅動：

*   **Engine 層 — `AppLocale`**（[src/app/core/constants/locales/locale.interface.ts](src/app/core/constants/locales/locale.interface.ts)，由 `outputLanguage` 驅動）：放 LLM-bound 的 prompt 片段、JSON Schema 描述、檔名對應，以及任何會被寫進 chat 內容（必須跟存檔語言對齊）的字串。透過 `getLocale(outputLanguage)` 查找，由 services / engines 消費。
*   **UI 層 — `I18nService` + dotted-key 字典**（[src/app/core/i18n/](src/app/core/i18n/)，由 `interfaceLanguage` 驅動）：放對話框標題、按鈕、錯誤訊息 —— 所有即時渲染給 user 的文字。透過 `translate` pipe / directive 或 `i18n.translate('ui.X')` 消費。

### 已內建支援的語言

| 層 | 語言 | 註冊位置 |
| :--- | :--- | :--- |
| Engine（`AppLocale`） | 繁體中文（`zh-TW`，亦為 `default` fallback）、英文（`en-US`） | [src/app/core/constants/locales/index.ts](src/app/core/constants/locales/index.ts) — `LOCALES` map |
| UI（`I18nService`） | 繁體中文（`zh-TW`）、英文（`en`） | [src/app/core/i18n/ui-locales.ts](src/app/core/i18n/ui-locales.ts) — `UI_LOCALES` 陣列 |

Engine 層解析（[src/app/core/constants/locales/index.ts](src/app/core/constants/locales/index.ts)）：
*   `getLocale(lang)` — 回傳對應 `AppLocale`，找不到時依 `id` 比對，再不行則回 `'default'`。
*   `getLangFolder(lang)` — 回傳該 locale 的 `folder` 欄位，用來解析 `public/assets/system_files/<folder>/` 下分桶素材。
*   `getLanguagesList()` — 餵給 Settings 的 **Story Language** 下拉。

UI 層解析：`I18nService.currentLang()` 讀取 `AppConfigStore.interfaceLanguage()`，遇 `'system'` 就走註冊表 `matchPrefixes` 解析，再以 dotted key 走訪當前字典。新增 UI 語言只要在 `UI_LOCALES` 加一筆 + 寫一份字典檔；resolver 與 `AppConfigStore` 型別都不用動。

### 各層涵蓋範圍（自動本地化）

**Engine 層（`AppLocale`，隨 Story Language 切換）：**
...
| 分析軌跡的 markdown 標籤（會被存進 chat-message analysis 文字內） | `analysisTrace` |
| 包覆使用者輸入的 `<intent>` XML marker，以及載入舊存檔時做反向對應 | `intentTags` |
| Engine 寫成 chat 內容、或回送給 LLM 的字串（如 `INTRO_TEXT`、`LOCAL_INIT_ANALYSIS`、regenerate-save prompt 等） | `engineStrings` |

**UI 層（`I18nService` dotted-key namespace，隨 Interface Language 切換）：**

| Namespace | 涵蓋 |
| :--- | :--- |
| `ui.*` | 聊天區按鈕、snackbar、alignment 九宮格、批次搜尋取代、regenerate-save 對話、Calibrate 模式、Profile 管理、同步狀態文字、Turn-update 面板 |
| `intent.labels.*` / `intent.descriptions.*` | 意圖選單標籤（`Action` / `行動`）與 tooltip |
| `placeholder.*` | 各意圖對應的輸入 placeholder（`<textarea>` 內顯示文字） |
| `settings.*` | Settings 對話框的欄位 label 與 hint（`Interface Language`、`Story Language`、`Follow system` 等） |
| `dialog.*` | 共用對話框 —— confirm / prompt / Payload 預覽 / Auto-update / Advanced Sync / Sync-to-disk / Agent Console |
| `sidebar.*` | 側邊欄外殼 + 書本清單、檔案檢視器、成本預估、Create-Scene 精靈、New-Game 對話框、控制列、本地檔案同步 |
| `app.*` | App shell —— Setup-Required / Resuming-Session 阻擋層、KB 同步 loading 訊息、PWA 更新與雲端較新版 snackbar |
| `screensaver.*` | 閒置螢幕保護的 aria-label 與啟動提示文字 |

過大不適合放在字串內的系統提示詞素材，則放於 `public/assets/system_files/<folder>/`，透過 `getLangFolder()` 載入。
...
要為現有劇本新增語言版本：
1.  將劇本資料夾（如 `public/assets/system_files/scenario/demo_world/`）複製到同層的新資料夾。
...
新增語言通常需要動**兩層** —— Engine 層的 `AppLocale`（LLM 對接側）跟 UI 層的字典（介面渲染側）。

**Engine 層（`AppLocale`）：**
1.  **建立 locale 檔**：新增 `src/app/core/constants/locales/ja.ts`，匯出實作 `AppLocale` 全部欄位（見上方 engine 層表）的 `JA_JP_LOCALE`，挑一個穩定的 `id`（如 `ja-JP`）與 `folder`（如 `ja`）。
2.  **註冊**：在 [src/app/core/constants/locales/index.ts](src/app/core/constants/locales/index.ts) 的 `LOCALES` map 中加入該 locale，使用一個易讀的 key（例如 `'Japanese'`）。新增後即會透過 `getLanguagesList()` 自動出現在 Settings → Story Language 下拉中。
...
**UI 層（`I18nService`）：**
6.  **寫字典**：新增 `src/app/core/i18n/dictionaries/ja.ts`，匯出與既有 `zh-tw.ts` / `en.ts` 同樣 namespace（`ui.*` / `intent.*` / `placeholder.*` / `settings.*` / `dialog.*` / `sidebar.*` / `app.*` / `screensaver.*`）結構的 `TranslationDict`。漏譯的 key 會在 UI 上 fallback 為原始 dotted key（例如 `ui.START_GAME`），所以部分翻譯是**可見**的失敗，不會靜默。
7.  **註冊**：在 [src/app/core/i18n/ui-locales.ts](src/app/core/i18n/ui-locales.ts) 的 `UI_LOCALES` 加一筆：
    ```ts
    { id: 'ja', label: '日本語', matchPrefixes: ['ja'], dictionary: ja },
    ```
    新 id 自動成為 `interfaceLanguage` 的合法值、Settings 下拉自動有對應選項，`'system'` 解析也會把 `navigator.language` 開頭為 `ja` 的使用者帶到日文。Resolver 程式一行不動。
...
2.  點擊 **"New Game"** 按鈕。
3.  選擇劇本（Scenario）並填寫主角設定（或使用預設值）。
