...
| Namespace | 涵蓋 |
| :--- | :--- |
| `ui.*` | 對話框標題、按鈕、snackbar、alignment 九宮格、批次搜尋取代、regenerate-save 對話、Calibrate 模式、Profile 管理、同步流程、Settings 標籤 |
...
New Game 對話框的 Generate 分頁仍有硬編碼英文 label（`Quick Preset`、`Genre`、`Tone`、`World Setting`、`Identity / Role`、`Generate World`），位於 [new-game-dialog.component.html](src/app/features/sidebar/components/new-game-dialog/new-game-dialog.component.html)。它們尚未遷入新的 `ui.*` UI 字典；該分頁產生的 *內容* 仍會正確跟隨 Story Language。把這些 label 遷進字典是已知待辦項。
...
**UI 層（`I18nService`）：**
6.  **寫字典**：新增 `src/app/core/i18n/dictionaries/ja.ts`，匯出與既有 `zh-tw.ts` / `en.ts` 同樣 `ui.*` / `intent.*` / `placeholder.*` / `settings.*` namespace 結構的 `TranslationDict`。漏譯的 key 會在 UI 上 fallback 為原始 dotted key（例如 `ui.START_GAME`），所以部分翻譯是**可見**的失敗，不會靜默。
