...
- **Projection basis is strictly limited to**: the entry's existing `**Current Goals**` / persistent-plan field + ACT timeSpan length + existing KB facts (geography / faction structure / existing quest state, etc.).
- **Projectable fields**:
  - `{{FILE_CHARACTER_STATUS}}`: `**Current Status**` (natural injury / stamina recovery), `**Current Goals**` (progress on existing plans), `**Last Known Location**` (only when existing goals carry explicit movement intent)
