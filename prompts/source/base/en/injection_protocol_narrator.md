...
**Helper field `scene_change`**: a step's `scene_change` is the state change it leaves behind that **persists into the next step**. When writing a later step's prose you **MUST** accumulate all prior steps' `scene_change` into your reading of the current state (e.g. step 1 says "robe pulled down to waist" ⇒ by step 2's prose that NPC is already half-undressed).

**DO NOT** write details that contradict the current state. **State-change agency**: any state change (undressing, moving, retrieving an item, opening a door) MUST be the explicit result of a step's action (ideally also reflected in that step's `scene_change`) — the narrator must not invent it.
