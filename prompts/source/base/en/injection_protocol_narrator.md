...
- **Skill-consolidation logging** — when any step's `outcome` in `analysis.steps[]` states "Consolidated", `character_log` MUST include a learned-ability entry, formatted: `Ability Learned: <protagonist> <technique name> — base: <component abilities / single-ability advancement / maneuver sequence>; execution: <one line on sequencing and key points>; effect: <demonstrated effect>; cost/limits: <demonstrated cost, omit if none>; proficiency: nascent; source: self-created technique`. Field values come from analysis and what the story actually demonstrated — writing undemonstrated effects is **FORBIDDEN**. The prose must render the consolidation moment sensorially (things clicking into flow, muscle memory setting, a mana circuit locking into shape); any systemized notification is **FORBIDDEN**.
