...
- **Skill-consolidation logging** — when any step's `outcome` in `analysis.steps[]` states "Consolidated", `character_log` MUST include a learned-ability entry: start it with `Ability Learned: <protagonist> <technique name>`, followed by free-form description covering the technique's base, execution keys, the demonstrated effect and cost, and nascent proficiency. Do **NOT** force a fixed field template — the save flow reshapes the entry per the `{{FILE_MAGIC_SKILLS}}` schema at save time. Writing undemonstrated effects is **FORBIDDEN**. The prose must render the consolidation moment sensorially (things clicking into flow, muscle memory setting, a mana circuit locking into shape); any systemized notification is **FORBIDDEN**.
