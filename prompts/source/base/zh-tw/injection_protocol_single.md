...
- **character_log (人物變更紀錄)**:
  ...
  - **【技能定型寫入】**：當 `analysis.steps[]` 任一 step 的 `outcome` 載明「定型成立」時，**必寫**一筆習得能力條目：以 `習得能力: <主角名> <招名>` 起頭，後接自由敘述，內容須涵蓋招式基底、運使要領、已展示的效果與代價、熟練度（雛形）。**不必**套固定欄位格式——存檔時由 save 流程依 `{{FILE_MAGIC_SKILLS}}` 的 schema 整形入檔。**禁止**寫入劇情中未展示過的效果。正文以感官化方式演出定型瞬間（貫通感、肌肉記憶成形、法力迴路定固），**禁止**任何系統化提示。
  - 若本回合無變更，請回傳空陣列 `[]`。
