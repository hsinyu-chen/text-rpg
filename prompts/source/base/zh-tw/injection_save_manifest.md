...
> **替換型 hunk：一個欄位一個 hunk，嚴禁合併。** 當同一個條目有多個欄位變動（例如好感度、目前狀態、最後已知位置同時改），**每一個變動的欄位各自產出一個獨立的替換 hunk**，不可併成一個大 hunk。每個 hunk 的 `target` 只涵蓋該欄位那一行（或該欄位本身連續的數行），逐字對齊原檔。
>
> **`target` 必須是原檔中一段「連續、不跳行」的文字。** 嚴禁把不相鄰的數行拼成一個 `target`、中間略過未變動的行——那不是逐字原文，套用時錨點必定對不上。若想改的幾行之間夾著不變的行，就拆成多個 hunk，一處連續變動一個。
...
劇情綱要（編年史）檔：**每次存檔必定新增一個完整的 ACT 區塊**（一條新的 ACT 標題 + 其下的時間節點），涵蓋本 ACT 的關鍵轉折、衝突結果與重要台詞，沿用該檔既有的 ACT 標題格式與編年體例，編號接續檔中現有最大的 ACT 編號 +1。

- **必為「新增」型 hunk**：省略 `target`，把整個新 ACT 區塊追加在編年史區段末端。
- **嚴禁附加進舊 ACT**：不可用「替換」型 hunk 去改既有的 ACT 區塊，也不可把本 ACT 的時間節點塞進上一個 ACT 底下。每次存檔就是新的一幕，編年史永遠只往後長一個新區塊。

編年史 hunk 是整個 ACT 的總結而非單一 message 的事實，因此**省略 `sourceMessageIds`** — 把整 ACT 的 message id 全部列上也對下游一致性檢查層沒有訊號。
