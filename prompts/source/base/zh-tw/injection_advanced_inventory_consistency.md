...
- **`[inventory]` 條目** —— 主角擁有物的變動:金錢、據點/房產、寄存物資(→ `{{FILE_ASSETS}}`)+ 隨身物品(→ `{{FILE_INVENTORY}}`)。兩個檔都在你的 Job 1 範圍,都用 digest 核對對應的 hunk。
...
- **revise**:`{{FILE_INVENTORY}}` / `{{FILE_ASSETS}}` 的 hunk(Job 1);`{{FILE_TECH_EQUIPMENT}}` 的 hunk(Job 2 與主 LLM 重疊時);`{{FILE_WORLD_FACTIONS}}` 的 hunk(Job 2 與主 LLM 重疊時,**僅限「關鍵物品/聖器/遺物」條目**;勢力動態、世界事件、特殊材料等非物品 hunk **絕對不碰**)。
- **新增**:`{{FILE_TECH_EQUIPMENT}}` 的 hunk(主角持有物的詳細設定);`{{FILE_WORLD_FACTIONS}}` 的 hunk(僅限非主角持有的關鍵物品/聖器/遺物的詳細設定)。資產檔本身就是設定格式,不需新增詳細設定條目。

`{{FILE_PLANS}}`、`{{FILE_CHARACTER_STATUS}}`、`{{FILE_MAGIC_SKILLS}}`、`{{FILE_STORY_OUTLINE}}`、`{{FILE_BASIC_SETTINGS}}` 的 hunk **一律不碰**。

## 工作一 — 核對物品變動

逐條檢查 `{{FILE_INVENTORY}}` **和** `{{FILE_ASSETS}}` 的 hunk —— 兩者都是主角擁有物,都吃 digest 的 `[inventory]` 條目(隨身 vs 金錢/據點/寄存):

1. 在 digest 的 `[inventory]` 條目裡找對應(主角拿到 / 失去 / 數量改變;隨身物品對到 `{{FILE_INVENTORY}}` hunk,金錢/據點/寄存物資對到 `{{FILE_ASSETS}}` hunk)。
...
- **主角持有 / 發現 / 研發**的實體物品 → `{{FILE_TECH_EQUIPMENT}}`。
- **非主角持有**的劇情關鍵物品(NPC 持有的聖器/法寶、出土遺物、某勢力的信物)→ `{{FILE_WORLD_FACTIONS}}` 的「關鍵物品」範圍。
...
- `file`:目標檔名,逐字照填。
- `context`:定位用的標題路徑 breadcrumb,以 ` > ` 分隔;檔案根層級填空字串。
...
- `newHunks`:新增的 hunk 陣列(不含 id);只能 target `{{FILE_TECH_EQUIPMENT}}` 或 `{{FILE_WORLD_FACTIONS}}`。
