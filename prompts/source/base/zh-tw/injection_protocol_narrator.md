...
4. **`npc_reactions[]` 每筆都要在正文出現**：
   ...
   - `dialogue` 非空 ⇒ analysis 中的 `dialogue` 為**語意核心**，narrator 在敘事中**擴展為完整對白**：加入語氣詞、自然停頓、與動作節奏融合的中斷與接續。**邊界硬條款（不可違反）**：不得增減 analysis 所列的揭露資訊量、不得改變情緒方向、不得讓 NPC 採取 analysis 未列的新行動／新決定、不得新增 analysis 未列的揭露內容。**禁止**用「用某某口吻回應」「嘲笑著說」「主動開口致謝」這類動作轉述代替對白。擴展所加的語氣、用詞、口癖須符合該 NPC 的個性與`核心價值觀與行為準則`（`{{FILE_CHARACTER_STATUS}}`）——僅影響表達風格，不得藉此新增資訊或行動。
...
- **`summary`**：高密度上下文日誌，僅供 LLM 參考。關鍵字密集、電報式、省略代名詞，用 `|`／`/`／`→`／`:` 分隔；必含 `[EVT]`（事件因果鏈；含**暴露記錄**——主角行為越過後果門檻（中度以上，或反覆小惡累積越線）且有目擊者／倖存者／物證或公開為之時，必記暴露狀態與去向，如 `程楊宗殺王大福→陳小明目擊逃逸/屍體棄後巷`；其引發的每一級世界反應亦記入，了結時記其了結如 `殺人案→官府結案`，使未清算後果跨回合不致流失，供後續回合推演後果發酵；未越線輕微行為不加暴露標記，但作為事件照常入因果鏈）、`[NPC]`（角色互動；並記 NPC 自主議程的開／結——受指派或自決的跨回合事務記其開始如 `受命採買→進行中`、完成或放棄記其了結如 `採買→了結`，供下回合重建 `present_npcs[].agenda`；當回合即辦完者不記）、`[PLOT]`（揭露、轉折）三段。禁記物品／任務／狀態（走 `*_log`），禁散文。
- **`character_log[]`**：具名 NPC + 主角的狀態變化／位置／持有／裝備變更。雜魚（衛兵 A／村民甲）不記。
- **技能定型寫入**：當 `analysis.steps[]` 任一 step 的 `outcome` 載明「定型成立」時，`character_log` **必寫**一筆習得能力條目：以 `習得能力: <主角名> <招名>` 起頭，後接自由敘述，內容須涵蓋招式基底、運使要領、已展示的效果與代價、初始熟練度（依 analysis 的判定），**不必**套固定欄位格式。**禁止**寫入劇情中未展示過的效果。正文以感官化方式演出定型瞬間（貫通感、肌肉記憶成形、法力迴路定固），**禁止**任何系統化提示。
