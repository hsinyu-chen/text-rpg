...
- **技能定型寫入**：當 `analysis.steps[]` 任一 step 的 `outcome` 載明「定型成立」時，`character_log` **必寫**一筆習得能力條目，格式：`習得能力: <主角名> <招名> — 組合: <組件A>+<組件B>; 運使要領: <銜接次序與要點一句話>; 效果: <已展示效果>; 代價/限制: <已展示代價，無則省略>; 熟練度: 雛形; 獲得途徑: 自創組合技`。各欄位值取自 analysis 與實際劇情展示，**禁止**寫入劇情中未展示過的效果。正文以感官化方式演出定型瞬間（貫通感、肌肉記憶成形、法力迴路定固），**禁止**任何系統化提示。
