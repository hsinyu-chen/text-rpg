...
export const SEARCH_REPLACE_TOOL: LLMFunctionDeclaration = {
    name: 'searchReplace',
    description: 'Apply one or more pattern-based replacements to a file in one shot WITHOUT transferring the file body through context. For a single edit, pass a one-element replacements array. For multiple mechanical edits in the same file, pass them all in one call. Returns total replacement count and per-pattern details. Workflow: grep first to confirm match counts, then call with expectedTotalReplacements set to the sum (or set per-entry expectedCount) as a safety net. For non-trivial regex, run with dryRun=true first to preview before/after samples. PATTERN MUST BE SMALL — pass only the short substring you are actually changing or anchoring on, NEVER a whole section. REPLACEMENT IS WRITTEN VERBATIM — never abbreviate it with an elision placeholder ("...", "(rest unchanged)", "(其餘不變)", "// ... existing ..."); a placeholder becomes literal corruption in the file. Three idioms: (1) CHANGE a value — pattern is the old substring, replacement the new one, e.g. {pattern:"Gold: 50", replacement:"Gold: 75"}. (2) INSERT a line mid-section (insertIntoSection only reaches a section start/end, not the middle) — pick a SHORT existing line next to the insertion point as the anchor and set replacement to that anchor PLUS your new line: {pattern:"- Strength: 10", replacement:"- Strength: 10\\n- Dexterity: 8"} inserts below; put the new line first to insert above. The anchor MUST match exactly once — if the line recurs (two characters both have "- Strength: 10"), widen the anchor to include the heading or a neighbouring line until the multi-line pattern is unique, and pass expectedCount:1 so a non-unique anchor fails (file unchanged) instead of inserting everywhere. (3) REMOVE a line — put the line plus its trailing "\\n" in pattern and set replacement to "".',
    parameters: {
        type: 'object',
        properties: {
            reason: { type: 'string', description: REASON_DESC },
