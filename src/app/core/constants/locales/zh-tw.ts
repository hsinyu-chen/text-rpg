...
export const ZH_TW_LOCALE: AppLocale = {
    ...
    responseSchema: {
        ...
        inventory: "描述物品變化。格式：'[標籤] [名稱] / [數量/等級] ([狀態辨識])'。必須精確區分類別：獲得/失去/消耗/移入。嚴禁將存入儲存空間（如次元箱）誤標為消耗。檢查歷史紀錄。若無變化則返回 []。",
        quest: "描述任務進度。格式：'[標籤] [任務標題] ([具體進展描述])'。必須精確區分類別：新任務/任務更新/目標達成/計畫變更。檢查歷史紀錄。若無變化則返回 []。",
        world: "描述世界動向。格式：'[標籤] [名稱/事件] ([詳述說明])'。必須精確區分類別：新勢力/世界觀拓展/裝備科技開發/魔法開發。檢查歷史紀錄。若無變化則返回 []。"
    },
