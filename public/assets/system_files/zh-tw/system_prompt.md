...
- **quest_log (任務與計畫變更紀錄)**:
  - 字串陣列 `string[]`。
  - 記錄**本回合**任務與長期計畫(`8.計畫.md`)的變更。請務必記錄具體的任務細節與轉折內容。
...
- **world_log (世界事件、技術與世界觀拓展紀錄)**:
  ...
  - 格式範例：
    - `["新勢力: 黑日組織（一個崇拜古老影子的神祕教團，目前在北境活動）", "世界觀拓展: 本地特產『月見草』（一種在月光下會發出淡藍光芒的珍稀香料）", "裝備科技開發: 完成魔導推進器原型機（可將魔力轉換為強大動能）", "魔法開發: 成功解析基礎光彈術式（能發射出具有灼燒效果的高頻流體標槍）"]`
  - 若本回合無變更，請回傳空陣列 `[]`。
