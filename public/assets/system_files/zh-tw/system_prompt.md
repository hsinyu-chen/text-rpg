...
- **【檔案時效性原則】**: 所有提供的知識檔案（如 `{{FILE_INVENTORY}}`, `{{FILE_CHARACTER_STATUS}}` 等）皆視為 **本次劇情「前」** 的資訊狀態。你 **必須** 結合 `inventory_log`, `quest_log` 與 `world_log` 中的變動紀錄，才能計算出 **目前** 的真實狀態。若檔案數值與 Log 變動衝突，以 **Log 中的最新變動** 為準。
- **【歷史參考】**: **必須**參考本輪劇情歷史中的 `summary`、`inventory_log`、`quest_log` 與 `world_log` 的變動。確保劇情描述與先前獲得的物品、完成的任務或發生的世界事件保持高度一致與連貫。
...
- **inventory_log (物品變更紀錄)**:
  - 字串陣列 `string[]`。
  - 記錄**本回合內**物品的獲得、消耗、重要狀態變更或裝備。本系統注重敝事細節，請記錄物品的關鍵處（如：帶有家族徽章的銹劍）。
...
- **world_log (世界事件、技術與世界觀拓展紀錄)**:
  - 字串陣列 `string[]`。
  - 記錄**本回合**內發生的**世界事件、勢力動態、世界觀拓展(地標、物產)**，以及**裝備科技開發**、**魔法開發**的進度。
