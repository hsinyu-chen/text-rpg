...
1. **依 `analysis.steps` 順序**，每步寫一段散文。**不可重新排序、合併、跳過。**
2. **每步 ≥ 50 字**（不含對話原文）。step 是場景節拍不是動詞清單；要含動作細節、NPC 姿態表情、環境觸感、節奏轉換。剛好 50 字就跳下一步算失格——密度要對齊 1-call。
3. **`pc_dialogue` 非空時**，正文必須以引號完整引用該句原文。**禁止改寫、意譯、增刪字句**（除非 correction 明示）。
4. **`npc_reactions[]` 每筆都要在正文出現**：
   - `physical` ⇒ 寫進姿態／動作／表情／眼神
   - `dialogue` 非空 ⇒ **必須以引號完整引用台詞原文**。**絕對禁止**用「用某某口吻回應」「嘲笑著說」「主動開口致謝」這類**動作轉述**代替台詞——這是嚴重違規。schema 已給了你台詞，照搬。
...
- 寫到 `analysis.steps` **最後一筆**（破壞點 step）的後果即停止——含該步的 `outcome` 文字、`npc_reactions`、`object_reactions`，讓使用者看到前提如何破壞。
- **不要**寫主角接下來打算做什麼或說什麼——後續步驟已被程式截掉，**不存在**。
...
正確做法：只寫 `analysis.steps` 中**確實存在的** step。看不到的 step 不存在，不腦補。
