...
**關於 `present_npcs[].state`**：**戰爭迷霧 / 意識狀態**欄位，**不是情緒**。自由發揮但限於該範疇。常用 tag：`"昏迷"` / `"熟睡"` / `"麻痺"` / `"匿蹤"` / `"通訊"`（遠地通訊不在現場）；可自創同範疇短 tag（如 `"幻象"` / `"靈魂出竅"` / `"淺眠（巨響可醒）"`）。`""` = 清醒在場（預設）。**禁止**填情緒（如 `"敵意"` / `"溫柔"`）——那些屬於 `npc_reactions[].physical` 與 `motivation`。
