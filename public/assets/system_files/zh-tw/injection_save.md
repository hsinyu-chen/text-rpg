...
| `{{FILE_MAGIC}}` | 主角方的術式原理、施法流程、魔法效果 | 使用魔法的裝備、魔法道具 |
...
- **【狀態同步原則】**：目前提供的 `{{FILE_*}}` 皆為存檔點/劇本起始的舊資訊。你必須合併 `character_log`, `inventory_log`, `quest_log`, `world_log` 的累計變動，計算出 **「現在的正確狀態」** 後，再將其寫入下方的檔案更新指令（XML）中。
