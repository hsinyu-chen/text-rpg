...
- [ ] 劇情綱要已更新當前 ACT 事件
- [ ] **觀察/細看指令中臨時生成的新人事物細節**（如 NPC 外貌/背景、地點描述、物品設定、勢力資訊等）→ 已寫入對應檔案持久化

**若任何 log 條目缺少對應的 `<save>` 更新，你的輸出即為不完整且無效。**

> [!IMPORTANT]
> **【觀察新設定持久化】**：若本輪劇情中曾因 `觀察`/`細看` 等指令而臨時生成了原本不存在於設定檔案的新人事物細節（並寫入 `character_log` 或 `world_log`），你**必須**將這些新生成的設定**完整寫入**對應的知識庫檔案（人物 → `{{FILE_CHARACTER_STATUS}}`；世界觀/地點/物產/勢力 → `{{FILE_WORLD_FACTIONS}}`；裝備規格 → `{{FILE_TECH_EQUIPMENT}}`），以確保世界設定的連續性與一致性。**絕對禁止**讓這些臨時生成的細節只存在於 log 而未落檔。
