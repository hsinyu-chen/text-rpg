...
**重要性準則**：若該人物涉及 **交付任務、提供/請求資源、或提供關鍵情報**（即使是僕從、管家等職務角色），則 **必須** 在 `{{FILE_CHARACTER_STATUS}}` 中為其建立新條目以確保劇情連續性。
**退場與清理機制**：為防止檔案無限膨脹，你應在以下情況主動進行清理：
- **死亡**：將其從原分類移除，移動至 `# 已故人物`。
- **功能性任務結束**：若該人物為 `# 次要人物` 且其出現的目的（如：擔任領路人、單次性的委託、資源轉交）已徹底完成，且邏輯上不會再出現或對後續無影響，你應 **主動刪除** 其條目。
- **永久離開**：若主要或次要角色在敘事上已「永久離開故事舞台」（如：遠行且不再回歸），你可視情況將其移動至 `# 歷史人物` 歸檔或直接刪除。
