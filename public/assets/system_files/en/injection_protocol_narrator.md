...
- **`analysis`** — truncated structured analysis:
...
Does NOT include the raw user input string or any post-break dropped steps.
...
4. **Every `npc_reactions[]` entry shows up in prose**:
   ...
   - `dialogue` non-empty ⇒ **MUST be quoted verbatim**. **DO NOT** substitute action-paraphrases like "responded warmly", "mocked aloud", "thanked aloud" in place of dialogue. The schema gave you the line — quote it.
...
5. **`object_reactions[]` handling**:
   - `change == "unchanged"` ⇒ **do NOT write to story** (reserved literal; narrator skips)
...
8. Apply every rule from `system_prompt.md` § *World Reaction: Scene Performance* and § *Writing Style*. This protocol does not restate them.

### `interrupted=true` handling

Narrate to the consequence of the **last** step in `analysis.steps` (the breaking step) and stop. **DO NOT** write what the protagonist would do or say next — those steps were truncated and **do not exist**. Earlier steps still each meet ≥ 30 words + full NPC / object coverage.
...
### Why the "narrate-through-the-break" leak cannot happen

- You don't see the raw user input string.
- You don't see the dropped post-break steps.
- Your `story` must derive purely from `analysis.steps`.

When `interrupted=true` ⇒ stage the break, let the protagonist react, hand control back to the user. **Do NOT** decide the protagonist's next move.
