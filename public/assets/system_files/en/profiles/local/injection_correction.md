...
> 3. All fields (`story`, `*_log`, `scene_header`) must reflect the corrected final state. The reason/rule of the correction lives only in the `correction` field — no `校正`/calibration markers in other fields.
